 ...
 */
data class NotificationPayload(val state: InstallState, val settings: UserSettings, val animation: AnimationContext)

data class InstallState(val progress: ProgressEntity, val background: Boolean, val isSameState: Boolean)
...
data class AnimationContext(val fakeItemProgress: Float)
