...
class PermissionProviderImpl(private val capabilityProvider: DeviceCapabilityProvider) : PermissionProvider {
    ...
    override suspend fun isPermissionGranted(authorizer: Authorizer, packageName: String, permission: String): Boolean = withContext(Dispatchers.IO) {
        var isGranted = false
        useDirectPrivileged(
            isSystemApp = capabilityProvider.isSystemApp,
            authorizer = authorizer,
