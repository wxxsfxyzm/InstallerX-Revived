...
enum class UninstallErrorType(val legacyCode: Int, val stringResId: Int) {
    ...
    DEVICE_POLICY_MANAGER(
        PackageManagerHidden.DELETE_FAILED_DEVICE_POLICY_MANAGER,
        R.string.exception_uninstall_failed_device_policy,
    ),
    ...
    USED_SHARED_LIBRARY(
        PackageManagerHidden.DELETE_FAILED_USED_SHARED_LIBRARY,
        R.string.exception_uninstall_failed_used_shared_library,
    ),
    ...
    companion object {
        fun fromLegacyCode(code: Int): UninstallErrorType = entries.find { it.legacyCode == code } ?: UNKNOWN
    }
}
